...
class DatasetManager:
    ...
    def collate_fn(self, batch):
        """
        Custom collate function for variable-length waveforms from dict dataset.
        Each item in batch is a dict: {'feature': waveform, 'label': label, 'label_id': int}
        """
        features = [item['feature'].squeeze(0) for item in batch] 
    ...
    def get_dataloader(self, split: str) -> DataLoader:
        ...
        return DataLoader(dataset, batch_size=self.batch_size, shuffle=(split == "train"), num_workers=self.num_workers, collate_fn=self.collate_fn)