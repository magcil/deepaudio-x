...
class SimPool(BasePooling):
    """
    SimPool pooling as described in "Keep It SimPool: Who Said Supervised Transformers Suffer from Attention Deficit?"
    ...
    Atributes:
        ...
        gamma (float | None): Exponent for value scaling.
        use_beta (bool): Whether to include a learnable translation parameter.
    """
    ...
    def forward(self, x):
        ...
        qq = (
            self.wq(q).reshape(Bq, Nq, self.num_heads, dq // self.num_heads).permute(0, 2, 1, 3)
        )  # (Bq, Nq, dq) -> (B, num_heads, Nq, dq/num_heads)
        kk = (
            self.wk(k).reshape(Bk, Nk, self.num_heads, dk // self.num_heads).permute(0, 2, 1, 3)
